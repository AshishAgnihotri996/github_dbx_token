...
raw_file_df.count()
raw_file_df.display()
raw_file_df.printSchema()

# COMMAND ----------

from pyspark.sql.functions import to_timestamp,col,expr,try_to_timestamp

# COMMAND ----------


fire_df = raw_file_df.withColumn("AvailableDtTm",try_to_timestamp(col("AvailableDtTm"),"MM-dd-yyyy HH:mm:ss a")).withColumn("Zipcode",expr("cast(Zipcode as string)")).withColumn("FinalPriority",expr("cast( FinalPriority as string)"))
